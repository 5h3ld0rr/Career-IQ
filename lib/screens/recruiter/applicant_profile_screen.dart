...
class ApplicantProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String resumeUrl = candidateData['resumeUrl'] ?? user['resumeUrl'] ?? '';
      ...
      body: CustomScrollView(
        ...
        slivers: [
          ...
          SliverAppBar(
            ...
            title: Text(name, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 17)),
          ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // ── Hero card ──
                  _heroCard(context, initials, name, headline, stage, stageColor, resumeUrl, theme),
                  ...
                  if (email.isNotEmpty || phone.isNotEmpty || location.isNotEmpty)
                    _section(
                      ...
                      child: Column(
                        children: [
                          if (email.isNotEmpty)    _infoRow(context, Icons.email_rounded,       email),
                          if (phone.isNotEmpty)    _infoRow(context, Icons.phone_rounded,       phone),
                          if (location.isNotEmpty) _infoRow(context, Icons.location_on_rounded, location),
                        ],
                      ),
                    ),
                  ...
                  if (skills.isNotEmpty) ...[
                    _section(
                      ...
                      child: Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: skills.map((s) => _skillChip(context, s)).toList(),
                      ),
                    ),
                    const SizedBox(height: 16),
                  ],
                  ...
                  // ── No data fallback ──
                  if (bio.isEmpty && exp.isEmpty && skills.isEmpty &&
                      email.isEmpty && phone.isEmpty && location.isEmpty)
                    Center(
                      child: Padding(
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.description_rounded, size: 13, color: theme.colorScheme.primary),
                            const SizedBox(width: 6),
                            Text(
                              'View CV',
  ...
  Widget _infoRow(BuildContext context, IconData icon, String text) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(icon, size: 15, color: theme.colorScheme.onSurface.withValues(alpha: 0.4)),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              text,
  ...
  Color _stageColor(String stage) {
    switch (stage) {
      ...
      case 'Interviewing': return Colors.orange;
