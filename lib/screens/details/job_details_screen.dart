              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: CachedNetworkImage(imageUrl: job.logoUrl, width: 80, height: 80),
                  ),
                  const SizedBox(height: 16),
                  Text(job.title,
                      style: Theme.of(context).textTheme.displaySmall?.copyWith(fontSize: 24, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text('\${job.companyName} • \${job.location}', style: Theme.of(context).textTheme.bodyLarge),
                  const SizedBox(height: 16),
                  Wrap(
                    spacing: 12,
                    children: [
                      _buildBadge(job.jobType, AppTheme.secondaryBlue, AppTheme.primaryBlue),
                      _buildBadge(job.salary, const Color(0xFFF0FFF4), AppTheme.success),
                    ],
                  ),
                ],
              ),
            ...
            return ElevatedButton(
              onPressed: () async {
                ...
                try {
                  ...
                  await jobProvider.applyForJob(auth.userEmail ?? 'unknown_user', job.id);
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Failed to submit application: $e'), backgroundColor: AppTheme.error),
                    );
                  }
                }
              },
