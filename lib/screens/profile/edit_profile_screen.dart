...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void _showPhoneVerificationDialog(AuthProvider auth) {
    // If the phone field has a value, use it as the default
                  ...
                  : () async {
                      if (!codeSent) {
                        if (phoneController.text.isNotEmpty) {
                          final success = await auth.sendPhoneOtp(phoneController.text);
                          if (success) {
                        ...
                        if (otpController.text.length == 6) {
                          final verified = await auth.verifyPhoneOtp(otpController.text);
                          if (verified && context.mounted) {
                            // Sync the screen's controller with the verified number
  ...
  Widget build(BuildContext context) {
    ...
    final experienceLabel = isRecruiter ? 'Industry / Sector' : 'Current Role / Experience';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: Transform.scale(
                  ...
                  child: _buildVerificationBadge(
                    title: (isExternal || (auth.isEmailVerified && _emailController.text == auth.userEmail)) ? 'Verified' : 'Verify',
                    isVerified: isExternal || (auth.isEmailVerified && _emailController.text == auth.userEmail),
                    onTap: (isExternal || (auth.isEmailVerified && _emailController.text == auth.userEmail))
                      ? () {}
                      : () {
                          auth.sendEmailVerification(newEmail: _emailController.text);
                        },
                  ),
                ),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
            ...
            // Phone Field (Editable/Verifiable)
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: Transform.scale(
                  scale: 0.8,
                  child: _buildVerificationBadge(
                    title: (auth.isPhoneVerified && _phoneController.text == auth.phoneNumber) ? 'Verified' : 'Verify',
                    isVerified: (auth.isPhoneVerified && _phoneController.text == auth.phoneNumber),
                    onTap: () {
        ...
        child: Padding(
          ...
          child: ElevatedButton(
            ...
            onPressed: () async {
              ...
              // Reload from Firebase to get the freshest verification state
              await auth.reloadUser();

              final emailChanged = _emailController.text.trim() != (auth.userEmail ?? '');
              final phoneChanged = _phoneController.text.trim() != (auth.phoneNumber ?? '');

              if (emailChanged && !auth.isEmailVerified) {
                auth.showNotification('Please verify your new email before saving.', isError: true);
                return;
              }

              if (phoneChanged && !auth.isPhoneVerified) {
                auth.showNotification('Please verify your new phone number before saving.', isError: true);
                return;
              }
              ...
              if (context.mounted) {
                ...
                await jobProvider.loadJobs(location: _locationController.text.trim());
                Navigator.pop(context);
              }
            },
            child: auth.isLoading 
